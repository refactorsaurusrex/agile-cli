...
if ($env:APPVEYOR_REPO_TAG -eq 'true') {
  ...
  Publish-Module -NuGetApiKey $env:psgallery -Path .\publish\AgileCli
  ...
  $options = @{
    Name = 'MyGet'
    SourceLocation = 'https://www.myget.org/F/agile-cli/api/v2'
    PublishLocation = 'https://www.myget.org/F/agile-cli/api/v2/package'
    InstallationPolicy = 'Trusted'
  }
  Write-Host "Registering the MyGet Gallery..."
  Register-PSRepository @options
  Write-Host "Publishing to the MyGet Gallery..."
  Publish-Module -NuGetApiKey $env:myget -Path .\publish\AgileCli -Repository MyGet
